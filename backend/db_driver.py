# connecting to a sqlite3 local database
import sqlite3
...
# representing the information i want to store in my database.
@dataclass
class Car:
    vin: str
    make: str
...
class DatabaseDriver:
    def __init__(
        self, db_path: str = "auto_db.sqlite"
    ):  # database path where i want to save this
    ...
    def get_car_by_vin(self, vin: str) -> Optional[Car]:
        with self._get_connection() as conn:  # get the connection
            cursor = conn.cursor()
